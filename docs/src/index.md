...
Numbers (including boolean values), symbols, and string values are
printed as is. Values of specific types, such as `missing` are wrapped
in a `<span>` tag with a `class` attribute including the module.
